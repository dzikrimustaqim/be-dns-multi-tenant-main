const KategoriAlat = require('../models').KategoriAlat;
const Damage = require('../models').Damage;
const Kamar = require('../models').Kamar;
const PeriodeKamar = require('../models').PeriodeKamar;
...
module.exports = {
  listKategoriAlat(req, res) {
    return req.db.KategoriAlat
      .findAll({        
        order: [          
          ['createdAt', 'DESC'],
          ['createdAt', 'DESC'],
      ...
      .then((KategoriAlat) => res.status(200).send(KategoriAlat))
      .catch((error) => { res.status(400).send(error); });
  },

  //TODO: with search and relation
  listDamage(req, res) {
    return req.db.Damage
      .findAll({        
        order: [          
          ['createdAt', 'DESC'],
          ['createdAt', 'DESC'],
      ...
      .then((Damage) => res.status(200).send(Damage))
      .catch((error) => { res.status(400).send(error); });
  },
  ...
  addKategoriAlat(req, res) {
    ...
    return req.db.KategoriAlat
      ...
      .then((KategoriAlat) => res.status(201).send(KategoriAlat))
      .catch((error) => res.status(400).send(error));
  },

  updateKategoriAlat(req, res) {
    ...
    return req.db.KategoriAlat
      ...
      .then(KategoriAlat => {
        ...
        return KategoriAlat
          .update({            
            nama: req.body.nama || KategoriAlat.nama,                        
            status: req.body.active || KategoriAlat.status
          })
          .then(() => res.status(200).send(KategoriAlat))
          .catch((error) => res.status(400).send(error));
      })
      .catch((error) => res.status(400).send(error));
  },
